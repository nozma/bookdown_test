...
`_bookdown.yml`でも指定できるが、最初のRmdファイルのYAMLメタデータでPandoc関連の設定を指定することもできる。このファイルの指定例を示す。(`site: bookdown::bookdown_site`はRmdファイルで指定する必要がある？)
...
- docsフォルダに出力する必要があるため、YAMLメタデータか`_bookdown.yml`に`output_dir: docs`を指定しておく。
- Buildペインの`Build Book`ボタンを押す。
- GitHubのリポジトリを開き、Settingsタブの下部にあるGitHub Pagesの部分でSourceにmaster branch /docs folderを指定。